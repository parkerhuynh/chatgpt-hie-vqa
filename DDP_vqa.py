...
def main(args):
            ...
            if val_accuracy > best_acc:
                ...
                if rank == 0:
                    ...
                    if args.wandb:
                        ...
                        wandb.save(os.path.join(args.result_path, "test_predictions.csv"), base_path=args.result_path, policy = "now")
                        wandb.save(os.path.join(args.result_path, "val_predictions.csv"), base_path=args.result_path, policy = "now")
                        wandb.save(os.path.join(args.result_path, f"epoch_{epoch}_val.json"), base_path=args.result_path, policy = "now")
                        wandb.save(os.path.join(args.result_path, f"epoch_{epoch}_test.json"), base_path=args.result_path, policy = "now")
    ...
    if rank ==0 and args.wandb:
        wandb.save(os.path.join(args.result_path, "model.pth"), base_path=args.result_path, policy = "now")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--layer', type=int, choices=[1, 2], default=1,
                    help=f'Choose model: {model_dict}')
    parser.add_argument('--architecture', type=int, choices=[0, 1], default=1,
                    help=f'architecture')
    parser.add_argument('--version', type=str)
    args = parser.parse_args()
    

    #SET direction path to save result
    ...
    if "hie" in args.model_name.lower():
        args.answer_dict = f"./datasets/hie_answer_dicts_{args.dataset}_{layer_name}.json"
        temp_result_path = f"./tem_results/{args.dataset}/{args.model_name}/layer_{layer_name}-architecture{args.architecture}-{args.version}"
