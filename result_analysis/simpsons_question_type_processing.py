...
def question_type_processing(question_type):
    ...
    # # 
    for word in ["clothing", "object", "material", "shape", "food", "transportation", "pattern", "letter", "drink", "landform", "transport"]:
        if word in question_type:
            return "object"
    for word in ["age", "animal", "body parts", "gender", "body part", "person", "emotion", "relationship", "facial expression"]:
        if word in question_type:
            return "human"
    for word in ["weather", "season", "time", "unknown", "any", "not", "event", "geometry", "music", "description", 
                 ...
                 "orientation", "facial feature","please provide the question type for the question what does it say after water lane?", "size", "style", "attribute"]:        
        if word in question_type:
            return "other"
    ...
    for word in ["color"]:   
        if word in question_type:
            return "color"
    for word in ["location", "hand dominance", 'distance']:   
        if word in question_type:
            return "location"
